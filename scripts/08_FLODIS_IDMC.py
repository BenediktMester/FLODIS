...
def main():
    ...
    if IDMC_affected_shape.empty == True: 
        ...
        if IDMC_FL_ISO3 != 'GBR':  
           ...
           # print(IDMC_FL.loc['countries_forUKonly'])
            #print(pd.isna(IDMC_FL.loc['countries_forUKonly'])) 
    ...
    if IDMC_affected_shape.empty != True: 
        ...
        if len(DFO_satellite_shape) > 0:   
           ...
           # DFO_satellite_shape.to_file(f"{path_run}temp/{split_nr}_final.shp")          
    ...
    IDMC_FL['GFD_matches'] = DFO_match_list  
    IDMC_FL['GFD_matches_nr'] = DFO_match_nr_list  
    IDMC_FL['GFD_matches_time_dif'] = DFO_match_time_dif_list 
    IDMC_FL['GFD_duration'] = DFO_duration_list 
 #   IDMC_FL['DFO_satellite_shape_area'] = DFO_satellite_shape_area_list
    IDMC_FL['matching_type'] = matching_type_list
    IDMC_FL['pop_affected_sum_GHSL'] = affected_GHSL_sum_list 
    IDMC_FL['pop_affected_mean_GHSL'] = affected_GHSL_mean_list 
    IDMC_FL['pop_density_GHSL'] = DFO_satellite_shape_pop_density_GHSL_list 
    IDMC_FL['pop_affected_sum_GPW'] = affected_GPW_sum_list 
    IDMC_FL['pop_affected_mean_GPW'] = affected_GPW_mean_list 
    IDMC_FL['pop_density_GPW'] = DFO_satellite_shape_pop_density_GPW_list 
